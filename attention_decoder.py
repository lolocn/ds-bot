...
def attention_decoder_fn_train(encoder_state,
                                                             ...
                                                             name=None):
    """
    训练时`dynamic_rnn_decoder`的注意力解码器方法

    `attention_decoder_fn_train` 是训练基于注意力机制的 sequence-to-sequence 模型的方法
    当`dynamic_rnn_decoder` 在训练模式时使用

    使用用户的参数调用`attention_decoder_fn_train`
    返回 `decoder_fn`, 传入
    `dynamic_rnn_decoder`, 
    例如
    ```
    ...
    def decoder_fn(time, cell_state, cell_input, cell_output, context_state):
        """ 训练时`dynamic_rnn_decoder` 的解码器方法 .
...
def attention_decoder_fn_inference(output_fn,
                                                                     ...
                                                                     name=None):
    """
    inference时`dynamic_rnn_decoder`的注意力解码器方法

    `attention_decoder_fn_inference` 是inference基于注意力机制的 sequence-to-sequence 模型的方法
    当`dynamic_rnn_decoder` 在inference模式时使用

    使用用户的参数调用`attention_decoder_fn_inference`
    返回 `decoder_fn`, 传入
    `dynamic_rnn_decoder`, 
    例如
    ...
    def decoder_fn(time, cell_state, cell_input, cell_output, context_state):
        """inference时`dynamic_rnn_decoder` 的解码器方法.
...
def prepare_attention(attention_states,
    ...
    # attention_states准备注意力 keys / values 
