...
def prepare_data(path, is_train=True):
    ...
    with open('%s/resource.json' % path) as f:
...
def build_vocab(path, raw_vocab, trans='transE'):
    ...
    entity_relation_embed = np.array(
        entity_embed + relation_embed, dtype=np.float32)
...
def generate_summary(model, sess, data_train):
    selected_data = [random.choice(data_train)
                     for i in range(FLAGS.batch_size)]
    batched_data = gen_batched_data(selected_data)
    summary = model.step_decoder(
        sess, batched_data, forward_only=True, summary=True)[-1]
    return summary
...
'''
自动测试模型效果
读取最近一次的checkpoint用测试集进行测试
输出每条测试数据生成的response、困惑度、知识使用率到test.log
输出整体的困惑度、知识使用率到test.res
'''
    ...
    with open('%s.res' % FLAGS.inference_path, 'w') as resfile, open('%s.log' % FLAGS.inference_path, 'w') as outfile:
        for step in [step for step in steps if step > low_step and step < high_step]:
            ...
            for post, response, result, match_triples, triples, entities in zip([data['post'] for data in data_dev], [data['response'] for data in data_dev], results, [data['match_triples'] for data in data_dev], [data['all_triples'] for data in data_dev], [data['all_entities'] for data in data_dev]):
                ...
                triples = [csk_triples[tri]
                           for triple in triples for tri in triple]
                match_triples = [csk_triples[triple]
                                 for triple in match_triples]
                entities = [csk_entities[x]
                            for entity in entities for x in entity]
                matches = [x for triple in match_triples for x in [
                    triple.split(', ')[0], triple.split(', ')[2]] if x in response]

                for word in result:
                    if word not in stopwords and word in entities:
                        result_matched_entities.append(word)
                outfile.write("post: %s\nresponse: %s\nresult: %s\nmatch_entity: %s\n\n" % (" ".join(
                    post), " ".join(response), " ".join(result), " ".join(result_matched_entities)))
                match_entity_sum[setidx] += len(set(result_matched_entities))
                cnt += 1
            match_entity_sum = [
                m / setnum for m in match_entity_sum] + [sum(match_entity_sum) / len(data_dev)]
            losses = [np.sum(loss[x:x+setnum]) / float(setnum)
                      for x in range(0, setnum*4, setnum)] + [np.sum(loss) / float(setnum*4)]
        ...
        while True:
            ...
            while st < train_len:
                ...
                for batch in range(st, ed, FLAGS.batch_size):
                    loss_step += train(model, sess,
                                       data_train[batch:batch+FLAGS.batch_size]) / (ed - st)

                def show(a): return '[%s]' % (
                    ' '.join(['%.2f' % x for x in a]))
                ...
                summary = tf.Summary()
                summary.value.add(tag='decoder_loss/train',
                                  simple_value=loss_step)
                summary.value.add(tag='perplexity/train',
                                  simple_value=np.exp(loss_step))
                summary_writer.add_summary(summary, model.global_step.eval())
                summary_model = generate_summary(model, sess, data_train)
                summary_writer.add_summary(
                    summary_model, model.global_step.eval())
                evaluate(model, sess, data_dev, summary_writer)
                previous_losses = previous_losses[1:]+[np.sum(loss_step)]
                loss_step, time_step = np.zeros((1, )), .0
                st, ed = ed, min(
                    train_len, ed + FLAGS.batch_size * FLAGS.per_checkpoint)
